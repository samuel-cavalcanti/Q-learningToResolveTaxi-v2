...
class GeneticAlgorithm:
    ...
    def __init__(self, size_population, interval, type_individual, fitness_function, minimization=True):
        ...
        self._getFirstBestIndividuals()

        # print("best_children score {} , variance population {}".format(self.best_individuals.score,
        #                                                                self._variancePopulation()))

    def _getFirstBestIndividuals(self):
    ...
    def _generateRandomPopulation(self, size_population, type_individual):
        ...
        if type_individual == float and type(self._interval) == np.ndarray:
    ...
    def _getIntevalSize(self):
        new_size = 1
        for size in self._removeLastDim(self._interval.shape):
            new_size *= size
        return new_size

    def _gerateRandom_Sample(self):
        new_size = self._getIntevalSize()
        new_shape = self._removeLastDim(self._interval.shape)

        interval = self._interval.reshape(new_size, 2)
    ...
    def _evaluatePopulation(self, population):
        ...
        # head_index = int((i - 1) * size / 3)
        # tail_index = int(i * size / 3)
        # chromosome_list = [individuals.chromosome for individuals in population[head_index:tail_index]]
        # scores = pool.map(self.fitness_function, chromosome_list)
        #
        # for score, individual, chromosome in zip(scores,
        #                                          [individuals for individuals in population[head_index:tail_index]],
        #                                          chromosome_list):
    ...
    def execute(self, variance=0.00001, max_iterations=float("inf")):

        iteration = 0

        while self._variancePopulation() > variance and iteration < max_iterations:
            ...
            # if self.best_individuals.score >= 9:
            #     self.best_individuals.save("best_table")
